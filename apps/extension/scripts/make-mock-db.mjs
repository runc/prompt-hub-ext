...
DELETE FROM prompts;
  ...
  const imagePool = [
    "https://picsum.photos/seed/prompt-hub-1/1200/800",
    "https://picsum.photos/seed/prompt-hub-2/1200/800",
    "https://picsum.photos/seed/prompt-hub-3/1200/800",
    "https://picsum.photos/seed/prompt-hub-4/1200/800",
    "https://picsum.photos/seed/prompt-hub-5/1200/800"
  ]
  ...
  for (let i = 0; i < 48; i++) {
    ...
    const variables = JSON.stringify(
      Object.fromEntries(extractVariables(tpl.content).map((name) => [name, ""]))
    )
    ...
    const images =
      rand() < 0.35
        ? JSON.stringify(
            Array.from({ length: 1 + Math.floor(rand() * 3) }, (_, j) =>
              pick(imagePool, i + j)
            )
          )
        : null
    ...
    const videos =
      rand() < 0.2 ? JSON.stringify([pick(videoPool, i)]) : null

    rows.push({ title, content, category, tags, variables, images, videos })
  }

  const insertSql = rows
    .map(
      (r) =>
        `INSERT INTO prompts (title, content, category, tags, variables, images, videos) VALUES ('${escapeSqlString(
          r.title
        )}', '${escapeSqlString(r.content)}', '${escapeSqlString(
          r.category
        )}', '${escapeSqlString(r.tags)}', '${escapeSqlString(r.variables)}', ${
          r.images == null ? "NULL" : `'${escapeSqlString(r.images)}'`
        }, ${r.videos == null ? "NULL" : `'${escapeSqlString(r.videos)}'`});`
    )
    .join("\n")
